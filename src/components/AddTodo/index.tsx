...
const AddTodo: React.FC<AddTodoProps> = ({
  ...
  const onContentInputChange = (e: React.FormEvent<HTMLInputElement>): void =>
    setContent(e.currentTarget.value);

  const [priority, setPriority] = useState<TodoPriorityEnum | null>(
    TodoPriorityEnum.LOW
  );
  const PriorityToSliderValueMap = new Map<TodoPriorityEnum, number>([
    [TodoPriorityEnum.LOW, 1],
    [TodoPriorityEnum.MEDIUM, 2],
    [TodoPriorityEnum.HIGH, 3],
  ]);
  const onPrioritySliderChange = (value: number): void => {
    const SliderValueToPriorityMap = new Map<number, TodoPriorityEnum>([
      [1, TodoPriorityEnum.LOW],
      [2, TodoPriorityEnum.MEDIUM],
      [3, TodoPriorityEnum.HIGH],
    ]);
    setPriority(SliderValueToPriorityMap.get(value) || null);
  };

  const [categories, setCategories] = useState<Category[]>([]);
  const onCategoriesSelectChange = (categoriesIds: CategoryId[]): void =>
    setCategories(
  ...
  const onAddCategoryInputChange = (
    e: React.FormEvent<HTMLInputElement>
  ): void => setAddCategoryInput(e.currentTarget.value);
  ...
  const doFormFieldsValidate = (): boolean =>
    ...
    <React.Fragment>
          ...
          <div className={style.modalContainer} key="modal">
            <div>
              <div className={style.modalHeader}>
                <Typography.Title className={style.addTodoTitle}>
                  {"Add Todo"}
                </Typography.Title>
                <Button
              ...
              <div className={style.modalBody}>
                ...
                {InputGroup(
                  ...
                  <Slider
                    ...
                    value={
                      priority !== null
                        ? PriorityToSliderValueMap.get(priority)
                        : 0
                    }
            ...
            <div className={style.modalFooter}>
              <div className={style.doneTodoButtonContainer}>
                <Button
                  ...
                  className={
                    doFormFieldsValidate()
                      ? style.doneTodoButton
                      : style.doneTodoButtonDisabled
                  }
                >
                  {"Done"}
                </Button>
      ...
      <div className={style.addTodoButtonContainer}>
        <Button
          ...
          disabled={disabled}
          className={
            !disabled ? style.addTodoButton : style.addTodoButtonDisabled
          }
          type={"primary"}
          style={{
            margin: "10px",
