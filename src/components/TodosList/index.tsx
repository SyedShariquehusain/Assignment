...
const TodosList: React.FC<TodosListProps> = ({
  ...
  const [filterModalVisibility, setFilterModalVisibility] =
    React.useState<boolean>(false);

  const [filterByCategories, setFilterByCategories] = React.useState<
    Category[]
  >([]);
  const [filterByPriorities, setFilterByPriorities] = React.useState<
    Array<TodoPriorityEnum | "None">
  >([]);

  const composedTodos: TodoWithChildEntities[] = todos
    .filter((todo) => {
      if (filterByCategories.length > 0) {
        const commonCategories = filterByCategories
          .map((c) => c.id)
          .filter((id) => todo.categoriesIds?.includes(id));
    ...
    .map((todo) => {
      const todoWithCategories: TodoWithChildEntities = {
        ...
        categories: categories.filter((category) =>
          todo.categoriesIds?.includes(category.id)
        ),
      };
      return todoWithCategories;
    });

  const todosCategories: Category[] = useCategories(
    _.uniq(
      _.flatten(todos.map((todo) => todo.categoriesIds).filter(isNotEmpty))
    )
  );
  ...
  const onFilterChangesOkClick = (
    priorities: Array<TodoPriorityEnum | "None">,
    categories: Category[]
  ): void => {
  ...
  return (
    <div className={style.container}>
      <Typography.Title className={style.todosTitle}>
        {"Todos"}
      </Typography.Title>
      {!hasError ? (
        !isLoading ? (
          todos.length > 0 ? (
            <>
              <div>
                ...
                <div className={style.filterRow}>
                  <Button
                    onClick={onFilterTodosButtonClick}
                    size="small"
                    shape="circle"
                    icon={<FilterOutlined />}
                  />
              ...
              {todos.filter((todo) => todo.completed).length > 0 && (
                <>
                  ...
                  <List
                    className={style.todosList}
                    dataSource={composedTodos.filter(
                      (todo) => !!todo.completed
                    )}
                    renderItem={(todo): JSX.Element => (
                      <div className={style.todoContainer}>
            ...
            <Result status="404" subTitle="Todos you add will appear here" />
          )
        ) : (
          <>
            <Skeleton />
